    ...
    def defineud(self, channel, args):
        if len(args) >= 1:
            ...
            if definitionJSON["result_type"] != "no_results":
                ...
                Util().sendMessage(channel, "No urban dictionary definition found for \"{0}\". Wow.".format(self.wordToDefine))
        else:
            self.tooltip(channel, args = {"command": "defineud"})

    def tooltip(self, channel, args):
        Util().sendMessage(channel, "Usage: @{0} <word>".format(args["command"]))